package com.example.runitup.mobile.queue

object QueueNames {
    const val WAIT_LIST_JOB = "WAIT_LIST_JOB"
    const val NEW_RUN_JOB = "NEW_RUN_JOB"
    const val BAD_TOKEN_JOB = "BAD_TOKEN_JOB"
    const val JOINED_RUN_JOB = "JOIN_RUN_JOB"
    const val FIRST_SESSION_JOB = "JOIN_RUN_JOB"
    const val LOCATION_JOB = "LOCATION_JOB"
    const val NEW_USER_JOB = "NEW_USER_JOB"
    const val RUN_CONFIRMATION_JOB = "RUN_CONFIRMATION_JOB"
    const val RUN_PROCESS_PAYMENT = "RUN_PROCESS_PAYMENT"
    const val PAYMENT_FAILED_JOB = "PAYMENT_FAILED_JOB"
    const val RUN_CANCELLED_JOB = "RUN_CANCELLED_JOB"
    const val RUN_SESSION_PUSH_JOB =  "RUN_SESSION_PUSH_JOB"

}


